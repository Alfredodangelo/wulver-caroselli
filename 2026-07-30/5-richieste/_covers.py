#!/usr/bin/env python3
"""2 opzioni copertina per "5 richieste": mazzo di carte (i prompt) e orologio (tempo). Scontorno + aura arancione."""
...
TAIL = ("polished obsidian black with a chamfered edge and bright emerald green hex #5CFC6E glowing accents, premium "
        "photorealistic 3D product icon, soft studio key light from upper left, soft contact shadow, three quarter "
        "view slightly above, 100mm lens, crisp micro bevels, on a plain seamless neutral light grey studio "
        "background. No text, no letters, no numbers, no logo, no watermark.")
jobs = {
 "cov-deck":  f"A neat deck of rounded rectangular cards fanned out in a smooth arc, the top card lifted, {TAIL}",
 "cov-clock": f"A sleek modern round wall clock seen at a three quarter angle, its hands and hour markers glowing, {TAIL}",
}
...
sheet.save(f"{SC}/cover5_sheet.png"); print("SHEET OK")
